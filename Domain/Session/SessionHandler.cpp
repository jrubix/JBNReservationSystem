...
namespace Domain::Session
{
  ...
  std::unique_ptr<SessionHandler> SessionHandler::createSession(const UserCredentials &credentials)
  {
    ...
    try
    {
      ...
      if (credentials.userName == credentialsFromDB.userName && credentials.passPhrase == credentialsFromDB.passPhrase && std::any_of(credentialsFromDB.roles.cbegin(), credentialsFromDB.roles.cend(), [&](const std::string &role) { return credentials.roles.size() > 0 && credentials.roles[0] == role; }))
      {
        // 2) If authenticated user is authorized for the selected role, create a session specific for that role
        if (credentials.roles[0] == "Receptionist")
          return std::make_unique<Domain::Session::ReceptionistSession>(credentials);
        if (credentials.roles[0] == "User")
          return std::make_unique<Domain::Session::HotelGuestSession>(credentials);
        if (credentials.roles[0] == "Administrator")
          return std::make_unique<Domain::Session::AdministratorSession>(credentials);
        if (credentials.roles[0] == "Management")
          return std::make_unique<Domain::Session::ManagementSession>(credentials);
    ...
    {
    } // Catch and ignore this anticipated condition
